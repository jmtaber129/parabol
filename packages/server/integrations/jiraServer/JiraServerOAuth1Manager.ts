...
export default class JiraServerOAuth1Manager {
  ...
  constructor(serverBaseUrl: string, consumerKey: string, privateKey: string) {
    ...
    this.oauth = new OAuth({
      ...
      // bind privateKey instead of using key parameter because it gets URL formatted which breaks the private key
      hash_function: (baseString) =>
        crypto.createSign('RSA-SHA1').update(baseString).sign(privateKey).toString('base64')
    })
  }
